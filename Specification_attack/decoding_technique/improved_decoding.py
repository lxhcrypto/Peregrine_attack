...
nb_sigs = [3, 5, 7, 9, 11, 13, 15, 17, 20]  # The sample size S
nb_sigs = [nb_sigs[i] * (10**6) for i in range(len(nb_sigs))]

# The l2-norm of error of e, where e = b - round(b')
norm_array = np.fromfile('../data/norm_array_specification.ha', dtype=np.float64)
...
key_recovered = np.fromfile('../data/key_recovered_array_specification.ha', dtype=np.float64)
key_recovered = key_recovered.reshape((nb_keys, len(nb_sigs), nb_points, m))

# The covariance matrix of Peregrine-512 signatures
cov_array = np.fromfile('../data/cov_array_specification.ha', dtype=np.float64)
cov_array = cov_array.reshape((nb_keys, len(nb_sigs), 4, n))

# The public key of Peregrine-512
pk_array = np.fromfile('../data/pk_array_specification.ha', dtype=np.int64)
...
for nb_key in range(nb_keys):
    ...
    for idx, item in enumerate(nb_sigs):
        Cov = cov_array[nb_key][idx]
        CovMtx = cov_matrix(Cov)
        s = item
        counter_success = 0
        for nb_point in range(nb_points):
            ...
            # Gram matrix G is computed by covariance and 79/12 is derived from binomial and uniform distribution
            G = (12/79) * CovMtx
            # 1.3973187004607963 is obtained by curve fitting and
            # sigma is the std of error of w which is outputted by gradient descent
            sigma = 1.3973187004607963 / sqrt(s)
            C = sigma ** 2 * G
